...
module RoleRequirementSystem
  ...
  module RoleSecurityClassMethods
    ...
    #    require_role "admin", :only => :update, :unless => "current_usuario.authorized_for_listing?(params[:id]) "
    #
    # Valid options
    #
    ...
    # This is the core of RoleRequirement.  Here is where it discerns if a usuario can access a controller or not./
    def usuario_authorized_for?(usuario, params = {}, binding = self.binding)
      ...
      self.role_requirements.each{| role_requirement|
        ...
        passed = false
        roles.each { |role|
          passed = true if usuario.has_role?(role)
        } unless (! usuario || usuario==:false)
  ...
  module RoleSecurityInstanceMethods
    ...
    def check_roles       
      return access_denied unless self.class.usuario_authorized_for?(current_usuario, params, binding)
      
      true
    end
    
  protected
    # receives a :controller, :action, and :params.  Finds the given controller and runs usuario_authorized_for? on it.
    ...
    def url_options_authenticate?(params = {})
      ...
      klass.usuario_authorized_for?(current_usuario, params, binding)
    end
  end
end