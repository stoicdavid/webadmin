...
module RoleRequirementTestHelper
  ...
  # Makes sure a usuario can access the given action
  #
  # Example:
  #
  #   assert_usuario_can_access(:quentin, "index")
  # 
  def assert_usuario_can_access(usuario, actions, params = {})
    assert_usuario_access_check(true, usuario, actions, params)
  end
  
  # Makes sure a usuario cant access the given action
  #
  # Example:
  #
  #   assert_usuario_cant_access(:quentin, "destroy", :listing_id => 1)
  # 
  def assert_usuario_cant_access(usuario, actions, params = {})
    assert_usuario_access_check(false, usuario, actions, params)
  end
  
  # Check a list of usuarios against a set of actions with parameters.
  # 
  # Parameters:
  #   usuarios_access_list - a hash where the key is the label for a fixture, and the value is a boolean.
  ...
  # 
  # Example:
  #   assert_usuario_access(
  #     {:admin => true, :quentin => false }, 
  #     [:show, :edit], 
  #     {:listing_id => 1})
  def assert_usuarios_access(usuarios_access_list, actions, params = {})
    usuarios_access_list.each_pair {|usuario, access| 
      assert_usuario_access_check(access, usuario, actions, params)
    }
  end
  
  alias :assert_usuario_cannot_access :assert_usuario_cant_access

private
  def assert_usuario_access_check(should_access, usuario, actions, params = {})
    ...
    (Array===actions ? actions : [actions]).each { |action|
      ...
      login_as usuario
      if should_access
        assert request_passes_role_security_system?(action, params), "request to #{@controller.class}##{action} with usuario #{usuario} and params #{params.inspect} should have passed "
      else
        assert ! request_passes_role_security_system?(action, params), "request to #{@controller.class}##{action} with usuario #{usuario} and params #{params.inspect} should have been denied"
      end
    }
  end
