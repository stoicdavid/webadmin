class PacientesController < ApplicationController
  ...
  def index
    ...
    if !params[:search].nil? && !@usuario.has_role?('doctor')
      ...
      :conditions => ['nombre LIKE :str or nombre_2 LIKE :str or app_pat LIKE :str or app_mat LIKE :str',values], :page => params[:page],:per_page => 10)
    elsif params[:search] and @usuario.has_role?('doctor') and doctor = @usuario.doctor
      values={}
      nombre = "%#{params[:search]}%".split(/\s+/)
      nombre.each { |x| values[:str] = x}
      @pacientes = doctor.pacientes.paginate(:all,
      :conditions => ['nombre LIKE :str or nombre_2 LIKE :str or app_pat LIKE :str or app_mat LIKE :str',values], :page => params[:page],:per_page => 5,:group => "pacientes.id")
